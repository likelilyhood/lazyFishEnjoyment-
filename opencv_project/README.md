## 环境搭建
1. 需要明白用什么编译器进行编译，在进行项目搭建时，配置CMakeLists文件时，结束后，在进行项目测试的时候，发现编译器一直在头文件出进行报错，和ai交流后，提出了错误方案（捂脸），也就是试图配置对应的g++.json，进行配置后，依旧报错，强行通过终端调用g++编译了对应的程序。
2. 忽视了CMake 的使用，继续和ai进行交互，首先更正了CMakeLists的命名错误，其次在命令行中创建了build的文件夹，并且使用cmake . 完成了项目的配置，配置时和ai 沟通在List添加了opencv的地址，确保可以及时寻找并调用cv2的库函数，而后环境配置完成，大概用时2小时，在编译时发现仍有报错，不得其解，后续删除了有关c++的配置json文件后，报错消失。
## 学习途径
1. 首先尝试在b站寻找对应的cpp版本opencv教程，很不幸，大多为基于window系统py版教程，少有基于cpp版本的教程，发现华东师大博士版本，但是发现我不适应此种网课模式，遂转向寻找其他答案
2. 第二种方案我选择了opencv的官方文档，在github中完成对应下载，发现是英文版，继续深入寻找后，发现了对应的汉化文档，万分感谢DOC源计划，然后对该文档进行了粗略的浏览以及学习
3. 第三种在完成对应任务时，使用了ai，大大加强了程序进度，在使用ai的时候充分发现，想法，抑或说实现思路的重要性，在搭建具体程序时，完全可以通过ai完成大体代码完成，具体效果以及参数调节需要根据需求来调节，我多在官方文档查阅对应的参数有关的解释部分
## 一些问题的具体解决或者优化思路
 1. 在寻找红色边缘时，我的思路是根据hsv图像来提取出红色部分然后进行边缘提取，第一次处理的结果误差过大，仅仅有部分区域，应该时此时的边缘分割与原图仍混有不清，故而和ai沟通过后，采取了canny方法先行优化，再进行边缘描绘，效果极好。
 2. 提取高亮区域时，本是直接进行二值化，而后提取，发现有背景和高光的影响，使得结果较差，于是转换思路，先对图片进行了滤波操作，进而仔细调节了函数对应的参数，使得高亮区域更加明显。
 3. 最后我将展示、保存、显示等函数作了一个重新组合，简化了代码数量，并且将代码交由ai进行了格式以及部分流程的优化，使得代码的风格得到了优化。
# 后续为利用ai辅助生成的一部分笔记以及交互内容总结
